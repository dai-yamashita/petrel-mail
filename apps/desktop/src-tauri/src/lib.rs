...
pub fn run() {
    ...
    let (readers, readers_live) = match (
        ...
        (a, b, counts, open, index) => {
            let why = [a.err(), b.err(), counts.err(), open.err(), index.err()]
                ...
                .unwrap_or_default();
            log_sync(&format!(
                "reading connections unavailable, reads share the write connection: {why}"
            ));
            let stand_in = || match Store::open_in_memory() {
                Ok(s) => s,
                Err(e) => cannot_start("Its mailbox could not be opened.", &db, &e.to_string()),
            };
            (
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::mail::view_message_source,
            commands::mail::save_message_eml,
            commands::mail::eml_filename,
