...
impl ViewTokens {
    ...
    /// The same token serves the HTML document and its cid:/attachment/
    /// subresources; it is not consumed on resolve. When the map exceeds
    /// [`VIEW_TOKEN_CAP`], the oldest issued token is evicted.
    pub fn issue(&self, message_id: i64) -> String {
        ...
        if let Ok(mut guard) = self.issued.lock() {
            let (map, order) = &mut *guard;
            map.insert(token.clone(), message_id);
            order.push_back(token.clone());
            while order.len() > VIEW_TOKEN_CAP {
    ...
    pub(crate) fn resolve(&self, token: &str) -> Option<i64> {
        self.issued.lock().ok()?.0.get(token).copied()
    }
}
...
const HEIGHT_REPORTER: &str = include_str!("height_reporter.js");
...
fn document(body: &str, blocked_remote: usize, nonce: &str, theme: FrameTheme) -> String {
    ...
    let blocked_init = format!("var BLOCKED = {blocked_remote};");
    let reporter = format!("{blocked_init}\n{HEIGHT_REPORTER}");
    let banner = "";
...
mod tests {
    ...
    fn the_frame_reports_the_fitted_box_not_the_unscaled_document() {
        ...
        // The reporter shares the host's ceiling, and holds a height that
        // only ever grows by what the host just gave it.
        assert!(doc.contains("HEIGHT_CAP = 200000"), "{doc}");
        assert!(doc.contains("holdIfChasingTheWindow"), "{doc}");
    }
    ...
    #[test]
    fn view_tokens_evict_oldest_when_over_cap_and_recent_still_resolve() {
        use super::ViewTokens;

        let tokens = ViewTokens::new();
        let mut issued = Vec::with_capacity(300);
        for i in 0..300_i64 {
            issued.push(tokens.issue(i));
        }
        let live = issued
            .iter()
            .filter(|t| tokens.resolve(t).is_some())
            .count();
        assert!(live <= super::VIEW_TOKEN_CAP, "live={live}");
        assert_eq!(tokens.resolve(issued.last().expect("recent")), Some(299));
        assert_eq!(tokens.resolve(&issued[0]), None);
    }
}
