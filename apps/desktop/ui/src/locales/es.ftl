...
onb-direct = Su correo va directamente entre este equipo y { $provider }. No hay ningún servidor de Petrel en medio. No hay ningún servidor de Petrel en absoluto.
onb-stored = Los mensajes se guardan en este equipo para que la búsqueda y la lectura funcionen sin conexión. Puede eliminar la cuenta y su correo cuando quiera.
...
onb-app-password-help = { $provider } pide una contraseña de aplicación en lugar de la habitual. Cree una en sus ajustes de seguridad y péguela aquí. Petrel la guarda en el almacén de contraseñas del sistema, nunca en un archivo.
onb-app-password-link = Crear una contraseña de aplicación
onb-password-help = Petrel la guarda en el almacén de contraseñas del sistema, nunca en un archivo.
...
outbox-retrying = No se envió nada. Se reintenta en { $when }.
...
sidebar-folders-help = El número junto a cada carpeta. El orden se cambia en la barra lateral, arrastrando.
...
identity-alias-note = Una identidad por cuenta.
...
storage-on-this-computer = En este equipo
...
storage-remove-all-help = Borra el correo, los ajustes y las contraseñas guardadas de este equipo.
...
accounts-storage = { $count ->
    [one] { $count } mensaje almacenado.
   *[other] { $count } mensajes almacenados.
}
...
update-err-unsupported = Las actualizaciones no están disponibles para esta compilación. Las nuevas versiones están en la página de lanzamientos.
...
settings-backup-help = Un archivo con sus preferencias, cuentas, servidores, colores y firmas.
...
alert-sent-copy-failed = El mensaje se envió, pero no se pudo guardar una copia en la carpeta Enviados.
msg-body-failed = No se pudo abrir este mensaje.
quote-attribution = El { $date } a las { $time }, { $who } escribió:
quote-forwarded = ---------- Mensaje reenviado ----------
quote-from = De:
quote-date = Fecha:
...
sc-move-inbox = Mover a Entrada
sc-pop-out = Abrir en una ventana nueva
...
list-load-failed = No se pudo cargar este buzón
list-refresh-failed = No se pudo actualizar este buzón: { $error }
footer-move = mover
footer-search = buscar
search-chip-in = En { $where }
search-chip-from = De { $who }
search-chip-attachment = Con adjunto
search-chip-unread = Sin leer
search-chip-starred = Destacados
search-chip-snoozed = Pospuestos
search-chip-year = Este año
...
font-serif = Serif
font-mono = Ancho fijo
size-small = Pequeño
size-normal = Normal
size-large = Grande
size-huge = Enorme
onb-host-of = Servidor { $what }
onb-port-of = Puerto { $what }
msg-view-source = Ver el origen del mensaje
msg-save-eml = Guardar como .eml…
msg-saved-eml = Guardado
source-title = Origen del mensaje
source-copy = Copiar
source-copied = Copiado
source-copy-manual = Pulsa ⌘C
source-bytes = { $n } bytes, tal como se guardó.
source-bytes-capped = Mostrando los primeros { $shown } de { $total } bytes, tal como se guardó.
print-from = De
print-to = Para
print-cc = Cc
print-date = Fecha
