...
onb-direct = Your mail moves directly between this computer and { $provider }. There is no Petrel server in between. There is no Petrel server at all.
onb-stored = Messages are stored on this computer so search and reading work offline. You can remove the account and its mail at any time.
...
onb-app-password-help = { $provider } asks for an app password rather than your normal one. Create one in its security settings and paste it here. Petrel keeps it in your system’s password store, never in a file.
onb-app-password-link = Create an app password
onb-password-help = Petrel keeps it in your system’s password store, never in a file.
...
outbox-retrying = Nothing was sent. Trying again in { $when }.
...
sidebar-folders-help = The number beside every folder. Arranged in the sidebar itself, by dragging.
...
identity-alias-note = One identity per account.
...
storage-on-this-computer = On this computer
...
storage-remove-all-help = Deletes the mail, settings and saved passwords from this computer.
...
accounts-storage = { $count ->
    [one] { $count } message stored.
   *[other] { $count } messages stored.
}
...
msg-print = Print…
msg-view-source = View Message Source
msg-save-eml = Save as .eml…
msg-saved-eml = Saved
...
update-err-unsupported = Updates are not available for this build. New releases are on the releases page.
update-could-not-ask = Could not check
update-check-failed = Could not check for updates: { $error }
settings-backup = Settings backup
settings-backup-help = One file holding your preferences, accounts, servers, colors and signatures.
...
alert-sent-copy-failed = The message was sent, but a copy could not be saved to your Sent folder.
msg-body-failed = This message could not be opened.
quote-attribution = On { $date } at { $time }, { $who } wrote:
quote-forwarded = ---------- Forwarded message ----------
quote-from = From:
quote-date = Date:
...
triage-not-applied = Nothing to move.
triage-nothing-to-archive = Nothing to archive.
undo-done = Undone
...
sc-move-inbox = Move to Inbox
sc-pop-out = Open in a new window
...
list-refresh-failed = Could not refresh this mailbox: { $error }
footer-move = move
...
source-copy-manual = Press ⌘C
source-bytes = { $n } bytes, as stored.
source-bytes-capped = Showing the first { $shown } of { $total } bytes, as stored.
print-from = From
print-to = To
print-cc = Cc
print-date = Date
